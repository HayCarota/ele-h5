...
const topThreeServices = computed(() => {
  return props.data.services.slice(0, 3)
})
const reduction = computed(() => {
  const reduction = props.data.discounts.find((item) => item.type === DiscountEnum.Reduction)
  return reduction ? reduction.content : []
})
const reductionLabel = computed(() => {
  return reduction.value.map((item) => `满${item.if}减${item.count}`)
})
  ...
  <div class="shop-header">
    <div class="shop-header__info">
      <div class="info__left">
        <div class="shop-name op-eliipsis">
